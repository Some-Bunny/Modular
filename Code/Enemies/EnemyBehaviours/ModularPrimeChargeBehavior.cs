...
public class ModularPrimeChargeBehavior : BasicAttackBehavior
{
    ...
    public override void Start()
    {
        ...
        this.m_shadowTrail_2 = this.m_aiActor.GetComponent<ImprovedAfterImage>();
    ...
    public override ContinuousBehaviorResult ContinuousUpdate()
    {
        ...
        {
            if (!this.m_aiAnimator.IsPlaying(this.hitAnim) | !this.m_aiAnimator.IsPlaying(this.parryAnim))
            {
                return ContinuousBehaviorResult.Finished;
            }
        }
    ...
    private void FireCollision()
    {
        ...
        this.m_bulletSource.BulletManager = this.m_aiActor.bulletBank;
        this.m_bulletSource.BulletScript = this.CollisionBulletScript;
        this.m_bulletSource.Initialize();
    }
    private void FireInterrupt()
    {
        if (interruptScript == null) { return; }
        if (Parried == false) { return; }
        if (!this.m_bulletSource)
        {
            this.m_bulletSource = this.ShootPoint.GetOrAddComponent<BulletScriptSource>();
        }
        this.m_bulletSource.BulletManager = this.m_aiActor.bulletBank;
        this.m_bulletSource.BulletScript = this.interruptScript;
        this.m_bulletSource.Initialize();
    }
    ...
    private void OnCollision(CollisionData collisionData)
    {
        if (this.State == ChargeBehavior.FireState.Charging && !this.m_aiActor.healthHaver.IsDead)
        {
            if (collisionData.OtherRigidbody)
            {
                ...
                if (projectile)
                {
                    if (this.m_currentSpeed < this.chargeAcceleration / 3)
                    {
                        return;
                    }
            ...
            if (!string.IsNullOrEmpty(this.hitAnim) | !string.IsNullOrEmpty(parryAnim))
            {
    ...
    public void OnPostRigidbodyMovement(SpeculativeRigidbody specRigidbody, Vector2 unitDelta, IntVector2 pixelDelta)
    {
    ...
    private void BeginState(ChargeBehavior.FireState state)
    {
        ...
        else if (state == ChargeBehavior.FireState.Charging)
        {
            ...
            if (m_shadowTrail_2) { this.m_shadowTrail_2.spawnShadows = true; }
        ...
        else if (state == ChargeBehavior.FireState.Bouncing)
        {
            ...
            this.m_aiAnimator.PlayUntilFinished(Parried == true ? parryAnim : this.hitAnim, true, null, 1f, false);
            if (this.CollisionBulletScript != null && !this.CollisionBulletScript.IsNull)
            {
                this.FireCollision();
            }
        }
    ...
    public BulletScriptSelector interruptScript;
    ...
    private ImprovedAfterImage m_shadowTrail_2;
