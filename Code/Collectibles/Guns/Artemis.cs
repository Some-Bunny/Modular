...
namespace ModularMod
{
    public class Artemis : GunBehaviour
    {
        ...
        {
            Gun gun = ETGMod.Databases.Items.NewGun("Artemis", "artemis_mdlr");
            Game.Items.Rename("outdated_gun_mods:artemis", "mdl:armcannon_8_alt");
            var c = gun.gameObject.AddComponent<Artemis>();
            ...
            gun.shootAnimation = "artemis_fire";
            gun.reloadAnimation = "artemis_reload";
            gun.introAnimation = "artemis_intro";
            gun.chargeAnimation = "artemis_charge";
            ...
            foreach (ProjectileModule projectileModule in gun.Volley.projectiles)
            {
                ...
                projectile.AnimateProjectileBundle("longshotalt_idle", StaticCollections.Projectile_Collection, StaticCollections.Projectile_Animation, "longshotalt_idle",
                ...
                projectile.hitEffects.tileMapHorizontal = Toolbox.MakeObjectIntoVFX((PickupObjectDatabase.GetById(89) as Gun).DefaultModule.projectiles[0].hitEffects.enemy.effects.First().effects.First().effect);
                projectile.hitEffects.tileMapVertical = Toolbox.MakeObjectIntoVFX((PickupObjectDatabase.GetById(89) as Gun).DefaultModule.projectiles[0].hitEffects.enemy.effects.First().effects.First().effect);
                projectile.hitEffects.enemy = Toolbox.MakeObjectIntoVFX((PickupObjectDatabase.GetById(89) as Gun).DefaultModule.projectiles[0].hitEffects.enemy.effects.First().effects.First().effect);
                projectile.hitEffects.deathAny = Toolbox.MakeObjectIntoVFX((PickupObjectDatabase.GetById(89) as Gun).DefaultModule.projectiles[0].hitEffects.enemy.effects.First().effects.First().effect);
            ...
            gun.DefaultModule.customAmmoType = CustomClipAmmoTypeToolbox.AddCustomAmmoType("artemis_AAa", StaticCollections.Clip_Ammo_Atlas, "actualartemis_1", "actualartemis_2");
                    ...
                    if (gun.IsCharging)
                    {
                        ...
                        if (VFXActive != true)
                        {
                            ...
                            for (int i = 0; i < 5; i++)
                            {
                                ...
                                Color laser = new Color(0f, 1f, 0.05f, 1f);
                        ...
                        foreach (ProjectileModule projectileModule in this.gun.Volley.projectiles)
                        {
                            ...
                            if (proj != null)
                            {
                                proj.baseData.damage = (5.25f - (elapsed * 4));
                            }
