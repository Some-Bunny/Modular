...
namespace ModularMod
{
    public class BubbleBlast : DefaultModule
    {
        public static ItemTemplate template = new ItemTemplate(typeof(BubbleBlast))
        {
            Name = "Bubble Blast",
            Description = "Bubbles!",
            LongDescription = "Slightly decreases Accuracy\nand increases fire rate by 33% (+33% per stack hyperbolically). Projectiles will slow to a crawl after a moment. Reloading boosts all slowed projectiles and increases their damage by 33% (+33% per stack).",
        ...
        public static void PostInit(PickupObject v)
        {
            ...
            h.SetName("Bubble Blast " + h.ReturnTierLabel());
            h.SetDescription("Slightly decreases Accuracy, and increases fire rate by 33% (" + StaticColorHexes.AddColorToLabelString("+33% hyperbolically", StaticColorHexes.Light_Orange_Hex) + ").\nProjectiles will slow to a crawl after a moment.\nReloading boosts all slowed projectiles\nand increases their damage by 33% (" + StaticColorHexes.AddColorToLabelString("+33%", StaticColorHexes.Light_Orange_Hex) + ").");
        ...
        public class BoostProjectileComponent : MonoBehaviour
        {
            public void DoBoost()
            {
                ...
                self.baseData.force *= DamageBoost;
